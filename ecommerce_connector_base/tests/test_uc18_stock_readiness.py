...
class TestUC18StockReadiness(TransactionCase):
    ...
    Odoo 18 fixture notes:
      ...
      Warehouse scoping context key: warehouse_id (NOT warehouse).
    ...
    def test_02_no_warehouse_fails_closed_parks_order(self):
        """02. readiness_only + no warehouse = fail closed, order parked."""
        ...
        self.assertIn("no warehouse is configured", order.error_message)
    ...
    def test_03_sufficient_free_stock_allows_creation(self):
        ...
        self.assertEqual(order.state, "imported")
        self.assertTrue(order.sale_order_id)
        self.assertNotIn("Stock warning", order.error_message or "")

    def test_04_insufficient_stock_parks_with_descriptive_warning(self):
        """04. free_qty < ordered: parks order in pending_review with descriptive warning."""
        ...
        self.assertEqual(order.state, "pending_review")
        self.assertFalse(order.sale_order_id)
        self.assertIn("Stock warning", order.error_message)
        self.assertIn("UC18 Storable Widget", order.error_message)
        self.assertIn("ordered 2.0", order.error_message)
        self.assertIn("unreserved available 1.0", order.error_message)
        self.assertIn(self.warehouse.name, order.error_message)
    ...
    def test_05_all_stock_reserved_parks_order(self):
        ...
        self.assertEqual(order.state, "pending_review")
        self.assertFalse(order.sale_order_id)
        self.assertIn("unreserved available 0.0", order.error_message)
    ...
    def test_09_mixed_lines_one_short_parks_whole_order(self):
        ...
        self.assertFalse(order.sale_order_id)
        self.assertIn("UC18 Storable Widget", order.error_message)
        self.assertNotIn("UC18 Service Item", order.error_message)
    ...
    def test_11_stock_warning_distinct_from_warning_message(self):
        """11. Stock warning writes to error_message; existing warning_message remains."""
        self._set_free_stock(self.storable_product, 0.0)
        order = self._make_ready_order()
    ...
    def test_12_duplicate_product_lines_aggregated(self):
        ...
        self.assertEqual(order.state, "pending_review")
        self.assertFalse(order.sale_order_id)
        self.assertIn("ordered 4.0", order.error_message)
        self.assertIn("unreserved available 3.0", order.error_message)

    def test_13_warehouse_context_key_warehouse_id(self):
        """13. Stock in a different warehouse is not counted (uses warehouse_id context key)."""
    ...
    def test_15_retry_after_stock_resolved(self):
        ...
        # Retry clears error_message, transitions captured -> ready -> imported
    ...
    def test_16_action_retry_import_parks_again_if_still_short(self):
        """16. Retry when stock is still short re-parks the order; error_message updated."""
        self._set_free_stock(self.storable_product, 0.0)
        order = self._make_ready_order()
        order.with_user(self.manager).action_create_sale_order()
        self.assertEqual(order.state, "pending_review")
    ...
    def test_17_multi_company_stock_scoped_to_order_company(self):
        ...
        second_company = self.env["res.company"].create({"name": "UC18 Second Company"})
        second_wh = self.env["stock.warehouse"].search(
    ...
    def test_18_retry_notification_displays_warning(self):
        """18. The dictionary returned by action_retry_import contains the stock warning."""
        self._set_free_stock(self.storable_product, 0.0)
        order = self._make_ready_order()
        order.with_user(self.manager).action_create_sale_order()
        
        # Retry while still short
    ...
    def test_19_action_validate_clears_old_stock_warning(self):
        """19. Manual validate -> create flow clears error_message if stock is fixed."""
        self._set_free_stock(self.storable_product, 0.0)
        order = self._make_ready_order()
        order.with_user(self.manager).action_create_sale_order()
        self.assertEqual(order.state, "pending_review")
        ...
        # Manual validate moves to ready and clears error_message
        order.with_user(self.manager).action_validate()
        self.assertEqual(order.state, "ready")
        self.assertFalse(order.error_message)

        # Create Sale Order now succeeds
        order.with_user(self.manager).action_create_sale_order()
        self.assertEqual(order.state, "imported")
        self.assertTrue(order.sale_order_id)
